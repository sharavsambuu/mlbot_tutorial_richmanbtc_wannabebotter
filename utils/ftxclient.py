...
class FtxClient:
    ...
    def get_all_trades(self, market: str, start_time: float = None, end_time: float = None, debug = False) -> List:
        ids = set()
        limit = 100
        results = []
        while True:
            ...
            if len(response) < limit:
                break
            next_end_time = min(parse_datetime(t['time']) for t in response).timestamp()
            if next_end_time == end_time:
                end_time = end_time - 0.000001
            else:
